...
def index(request):
    if request.method == "POST":
        form = request.POST
        latest_question_list = Question.objects.filter(topic=form.get("SeleccionTopic", "")).order_by('-pub_date')
    else:
...
def detail(request, question_id):
     ...
     return render(request, 'polls/detail.html', {'title':'Resultados de la pregunta:','question': question, 'year':datetime.now().year})
